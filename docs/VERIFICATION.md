# Release verification — 6.1.0

The complete `scripts/check_all.py` suite passed in the working checkout and in
an isolated copy of the allowlisted export. No paid Jev calls were made. Local
HTTP integration used synthetic upstream responses; browser tests used real
WebGL in headless Chrome. The final compiler-extension correction was followed
by the focused release suite and rebuild; generated application artifacts remained
identical.

| Suite | Passed |
| --- | --- |
| Node engine, contracts, host biology and episode tests | 178 |
| Immutable reference kernel | 43 |
| Python server, budgets, Choice and release tests | 84 |
| Studio desktop/mobile, biological outcomes and offline player checks | 61 |
| Total named checks | 366 |
...
Desktop/mobile screenshots were inspected. The renderer source diff changes only
the removed sugar overlay/palette; original cell geometry is byte-identical.
...
No repository was created, no history was rewritten and no commit or push was
made. A project license has not yet been selected. Only the allowlisted export is
intended for the new repository; the working checkout may retain private local
configuration and historical artifacts outside the export.
