import {barrierDamage,cellDamage,formatReading,signalExplanation} from './tissue-readouts.mjs';
...
function show(instant=true){const f=episode.frames[index];view.setFrame(f,instant);$('signalNote').textContent=signalExplanation(f,view.options.signals);$('timeline').value=index;$('clock').textContent=`REPLAY · ${f.time_min} min`;$('barrier').textContent=`Barrier ${Math.round(f.metrics.barrier*100)}%`;$('activeHud').textContent=`${f.metrics.active} sensing change`;inspect();}
...
$('signalSelect').onchange=()=>{view.options.signals=$('signalSelect').value;view.fieldKey='';if(episode)$('signalNote').textContent=signalExplanation(episode.frames[index],view.options.signals);};
