...
await page.goto(base+'/');await page.locator('.tissue-card').last().scrollIntoViewIfNeeded();await page.screenshot({path:'/tmp/portal-tissues.png',fullPage:true});
...
await page.getByRole('button',{name:'Memory B · 63.0 h',exact:true}).click();if(await page.locator('#metricMemory').textContent()!=='3')throw Error('Memory');
await page.getByRole('button',{name:'First division · 64.0 h',exact:true}).click();if(await page.locator('#metricDivisions').textContent()!=='1')throw Error('Division');
await page.screenshot({path:'/tmp/portal-ln.png',fullPage:true});await page.locator('.brand').click();if(new URL(page.url()).pathname!=='/')throw Error('Return to portal');
...
console.log({gutNavigation:true,legacyReplay:true,lnNavigation:true,lnMemory:3,lnDivisions:1,returnToPortal:true,errors,forbidden});if(errors.length||forbidden.length)throw Error('Portal failures');await browser.close();
